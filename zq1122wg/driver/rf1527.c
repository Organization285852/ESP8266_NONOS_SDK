...
#define MAX_BYTES				8
#define SYS_H_MIN       230//300us
#define SYS_H_MAX       340//

#define SYS_L_MIN       7000//8300
#define SYS_L_MAX       11000

#define T_MIN                   800//1080us
#define T_MAX                   1600

#define END_L_MIN   7000 //77000us

unsigned int t, t_last = 0, TL, TH, temp;
unsigned char data_1527[MAX_BYTES];
u8 bits = 0, sys_ok = 0;
void rf1527_intr_handler(void  *agr)
{
    ...
    t = system_get_time();
    if(GPIO_INPUT_GET(GPIO_ID_PIN(RFIN_IO_NUM)))//上升沿
    {
        TL = t - t_last;
		t_last = t;//system_get_time();
      //  os_printf("%d %d\n",TH,TL);
        if(sys_ok == 1)
        {
			//if(TL >= END_L_MIN)
			 if((TH >= SYS_H_MIN) && (TH <= SYS_H_MAX) && (TL >= SYS_L_MIN) && (TL <= SYS_L_MAX))//数据同步
       	 {
       	 	sys_ok = 0;
       	 	ID = 0;
       	 	for(i=0;i<bits/8;i++)
				...
				os_printf("1527_data is %02X,bits:%d\n",ID,bits);
           	 return;
       	 }
            temp = TH + TL;
		...
		else
		{
	        if((TH >= SYS_H_MIN) && (TH <= SYS_H_MAX) && (TL >= SYS_L_MIN) && (TL <= SYS_L_MAX))//数据同步
	        {
	            bits = 0;
	            sys_ok = 1;
	            os_memset(data_1527,0,MAX_BYTES); //
	            return;
	        }
		}
    }
...
rf1527_init(void)
{
    ...
    ETS_GPIO_INTR_ATTACH(rf1527_intr_handler, NULL);
