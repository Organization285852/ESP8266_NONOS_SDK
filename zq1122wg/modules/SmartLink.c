...
SmartLink(void)
{
    ...
    smartconfig_set_type(
        SC_TYPE_ESPTOUCH_AIRKISS); //SC_TYPE_ESPTOUCH,SC_TYPE_AIRKISS,SC_TYPE_ESPTOUCH_AIRKISS
    smartconfig_start(smartconfig_done);
}

//////////////////////////////////////////////////////////scan wifi and link last wifi
os_timer_t scan_t;
...
scan_done(void *arg, STATUS status)
{
    ...
    if(os_strlen(sta_conf.ssid)<20)
    {
    	Debug_printf("Last Link AP's SSID:\r\n");
    }
	else
	{
		Debug_printf("Last Link AP's SSID over 20\r\n");
	}
    if (status == OK)
    {
...
SmartLinkBegin(void)
{
	...
	os_timer_arm(&scan_t,3000,1); // 3s重复循环
	/*
    //通过AIRKISS连接
    smartconfig_set_type(
